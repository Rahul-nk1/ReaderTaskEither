...
import { getItemWithCache, LocalStorageEnv } from "../localStorage/LocalStorage"
...
export const makeLocalStorageCacheService: R.Reader<LocalStorageEnv, CacheService> = (
  localStorageEnv
): CacheService => ({
  getWithCache: <RGet, EGet, A>(
    key: string,
    ...
    pipe(
      RTE.ask<RGet>(),
      RTE.chainTaskEitherKW((r) => getItemWithCache(key, codec, get)({ ...localStorageEnv, ...r }))
    ),
