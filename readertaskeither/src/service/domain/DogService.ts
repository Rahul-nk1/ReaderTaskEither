...
export const getBreeds: RTE.ReaderTaskEither<HttpClientEnv, HttpJsonError, Array<Breed>> = pipe(
  getJson("https://dog.ceo/api/breeds/list/all", decodeWithCodec(getBreedsResponseCodec)),
  RTE.map((response) =>
    pipe(
...
export const getBreedImagesWithCache = (
  breed: Breed
): RTE.ReaderTaskEither<HttpClientEnv & CacheServiceEnv, HttpJsonError, Array<Image>> =>
  getWithCache(`breedImages-${breed.name}`, t.array(imageCodec), getBreedImages(breed))
 ...
 */
export const makeBreedImageService: R.Reader<HttpClientEnv, BreedImageService<HttpJsonError>> = (
  env
) => ({
