...
import { appEnv, breedServiceEnv, cacheServiceEnv, httpClientEnv } from "../AppEnv"
import {
  AppEnvContext,
  useAppEnvReducer,
  useAppEnvRemoteData,
...
import { BreedService, getBreeds, getBreedsWithCache } from "../service/domain/DogService"
...
export const MainRTEWithGlobalDeps = () => {
  const [remoteData, setRemoteData] = useState<RD.RemoteData<HttpJsonError, Array<Breed>>>(
    RD.initial
  )
...
export const MainAppEnvRT = () => {
  const [breedsRD, setBreedsRD] = useState<RD.RemoteData<HttpJsonError, Array<Breed>>>(RD.initial)
